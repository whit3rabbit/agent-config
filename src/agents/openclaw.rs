...
//!
//! 1. **Prompt rules**: project-local fenced blocks in `AGENTS.md`.
//! 2. **Skills**: AgentSkills-compatible folders at `~/.openclaw/skills`
//!    (Global) or `<root>/.agents/skills` (Local).
//! 3. **MCP servers**: global OpenClaw JSON5 config at
//!    `~/.openclaw/openclaw.json`, under `mcp.servers.<name>`.
//!
//! Native OpenClaw plugin and hook-pack installation remains intentionally
//! deferred because upstream documents that as a CLI/plugin lifecycle rather
//! than a stable file-backed hook contract.
...
use crate::spec::{HookSpec, InstructionSpec, McpSpec, McpTransport, SkillSpec};
use crate::status::StatusReport;
use crate::util::{
    file_lock, fs_atomic, instructions_dir, mcp_json_map, md_block, ownership, safe_fs, skills_dir,
};
...
impl OpenClawAgent {
    ...
    fn openclaw_home_from_home(home: &Path) -> PathBuf {
        home.join(".openclaw")
    }
    ...
    /// reuses the existing `<root>/.agents/` skills directory so the
    /// sidecar lives next to other agent-config artifacts.
    fn instruction_config_dir(scope: &Scope) -> Result<PathBuf, AgentConfigError> {
        Ok(Self::require_local(scope)?.join(".agents"))
    }
...
impl Integration for OpenClawAgent {
...
impl McpSurface for OpenClawAgent {
    fn id(&self) -> &'static str {
        "openclaw"
    }
...
impl SkillSurface for OpenClawAgent {
    fn id(&self) -> &'static str {
        "openclaw"
    }
...
impl OpenClawAgent {
    fn inline_layout(
        ...
        Ok(instructions_dir::InlineLayout {
            ...
            host_file: Self::prompt_path(scope)?,
        })
    }
}

impl InstructionSurface for OpenClawAgent {
    fn id(&self) -> &'static str {
        "openclaw"
    }
...
fn openclaw_mcp_value(spec: &McpSpec) -> Value {
    let mut obj = Map::new();
    match &spec.transport {
        ...
        McpTransport::Http { url, headers } => {
            ...
            obj.insert("transport".into(), Value::String("streamable-http".into()));
            if !headers.is_empty() {
        ...
        McpTransport::Sse { url, headers } => {
            obj.insert("url".into(), Value::String(url.clone()));
            obj.insert("transport".into(), Value::String("sse".into()));
            if !headers.is_empty() {
                obj.insert("headers".into(), string_map_value(headers));
            }
        }
...
mod tests {
    ...
    #[test]
    fn install_rules_writes_agents_md_block() {
        let dir = tempdir().unwrap();
        let agent = OpenClawAgent::new();
        let scope = Scope::Local(dir.path().to_path_buf());

        agent
            .install(&scope, &rules_spec("alpha", "Use the test rules."))
            .unwrap();
    ...
    #[test]
    fn install_skill_writes_local_agents_skills() {
        let dir = tempdir().unwrap();
        let agent = OpenClawAgent::new();
        let scope = Scope::Local(dir.path().to_path_buf());
    ...
    #[test]
    fn global_skill_root_uses_openclaw_skills() {
        let home = PathBuf::from("/tmp/home");
        assert_eq!(
            OpenClawAgent::skills_root_from_home(&home),
            PathBuf::from("/tmp/home/.openclaw/skills")
        );
    }

    #[test]
    fn skill_owner_mismatch_is_refused() {
        let dir = tempdir().unwrap();
        let agent = OpenClawAgent::new();
        let scope = Scope::Local(dir.path().to_path_buf());
    ...
    #[test]
    fn install_mcp_reads_json5_and_preserves_user_entries() {
        let dir = tempdir().unwrap();
  ...
  mcp: {
    servers: {
      user: { url: 'https://example.com/mcp' },
    },
  },
...
"#,
        ...
        OpenClawAgent::install_mcp_config(&cfg, &mcp_spec("github", "myapp")).unwrap();

        let parsed: Value = serde_json::from_slice(&std::fs::read(&cfg).unwrap()).unwrap();
        assert_eq!(parsed["plugins"]["enabled"], json!(true));
        assert_eq!(
            parsed["mcp"]["servers"]["user"]["url"],
            json!("https://example.com/mcp")
        );
        assert_eq!(parsed["mcp"]["servers"]["github"]["command"], json!("npx"));
        assert_eq!(
            parsed["mcp"]["servers"]["github"]["env"]["FOO"],
            json!("bar")
        );
    }
    ...
    fn install_mcp_is_idempotent() {
        ...
        OpenClawAgent::install_mcp_config(&cfg, &spec).unwrap();
        let second = OpenClawAgent::install_mcp_config(&cfg, &spec).unwrap();
        assert!(second.already_installed);
    }
    ...
    fn uninstall_mcp_round_trip() {
        ...
        OpenClawAgent::install_mcp_config(&cfg, &mcp_spec("github", "myapp")).unwrap();
        let report = OpenClawAgent::uninstall_mcp_config(&cfg, "github", "myapp").unwrap();
        assert!(!report.removed.is_empty());
        assert!(!cfg.exists());
    }
    ...
    #[test]
    fn remote_mcp_mapping_uses_openclaw_transport_names() {
        let http = McpSpec::builder("docs")
            .owner("myapp")
            .http("https://example.com/mcp")
            .build();
        let sse = McpSpec::builder("events")
            .owner("myapp")
            .sse("https://example.com/sse")
            .build();

        assert_eq!(
            openclaw_mcp_value(&http)["transport"],
            json!("streamable-http")
        );
        assert_eq!(openclaw_mcp_value(&sse)["transport"], json!("sse"));
    }
}
