...
use crate::util::{
    file_lock, fs_atomic, instructions_dir, md_block, ownership, safe_fs, skills_dir, yaml_mcp_map,
};
...
impl HermesAgent {
    ...
    /// uses a `<root>/.hermes/` subdirectory so the sidecar does not clutter
    /// the project root next to the user-visible `.hermes.md` host file.
    fn instruction_config_dir(scope: &Scope) -> Result<PathBuf, AgentConfigError> {
        Ok(Self::require_local(scope)?.join(".hermes"))
    }
...
impl InstructionSurface for HermesAgent {
    fn id(&self) -> &'static str {
        "hermes"
    }
