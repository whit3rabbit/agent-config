//! Sourcegraph Amp CLI integration.
//!
//! Surfaces:
//!
//! 1. **Prompt rules**: fenced HTML-comment block in `AGENTS.md` (Amp falls
//!    back to `CLAUDE.md` when `AGENTS.md` is absent; this crate writes the
//!    canonical `AGENTS.md`).
...
impl SkillSurface for AmpAgent {
    fn id(&self) -> &'static str {
        "amp"
    }
...
impl InstructionSurface for AmpAgent {
    fn id(&self) -> &'static str {
        "amp"
    }
...
mod tests {
    ...
    fn install_writes_agents_md_block() {
        ...
        agent
            .install(&scope, &rules_spec("alpha", "Use Amp."))
            .unwrap();
        let body = std::fs::read_to_string(dir.path().join("AGENTS.md")).unwrap();
        assert!(body.contains("Use Amp."));
        assert!(body.contains("AGENT-CONFIG:alpha"));
    }
    ...
    fn install_mcp_writes_settings_json() {
        ...
        agent
            ...
            .unwrap();
        let v = read_json(&dir.path().join(".amp/settings.json"));
